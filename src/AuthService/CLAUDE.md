...
Граф знаний **один на весь репозиторий** (все сервисы + SharedKernel в одном графе) и лежит в `.claude/graphify-out/`. Отдельного графа под AuthService нет — см. дисциплину графа в [`.claude/CLAUDE.md`](../../.claude/CLAUDE.md).
...
Пересобрать или обновить граф — всегда по всему `src/`, а не по одному сервису: `/graphify src/` (incremental: `/graphify src/ --update`). AuthService внутри графа выделяется через community-разбиение.
